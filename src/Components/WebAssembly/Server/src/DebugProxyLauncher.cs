...
namespace Microsoft.AspNetCore.Builder
{
    internal static class DebugProxyLauncher
    {
        ...
        public static Task<string> EnsureLaunchedAndGetUrl(IServiceProvider serviceProvider, string browserUrl)
        {
            lock (LaunchLock)
            {
                if (LaunchedDebugProxyUrl == null)
                {
                    LaunchedDebugProxyUrl = LaunchAndGetUrl(serviceProvider, browserUrl);
                }
        ...
        private static async Task<string> LaunchAndGetUrl(IServiceProvider serviceProvider, string browserUrl)
        {
            ...
            var devToolsHost = GetDevtoolsHostFromBrowserQuery(browserUrl);
        ...
        private static string GetDevtoolsHostFromBrowserQuery(string browserUrl)
        {
            if (browserUrl != null)
            {
                var browser = new Uri(browserUrl);
                return $"http://{browser.Host}:{browser.Port}";
            }
